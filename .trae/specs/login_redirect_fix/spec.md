...
## Overview
- **Summary**: 修复用户端登录后异常跳转到商家登录页的问题，以及用户端接口403权限错误的问题
- **Purpose**: 确保用户能正常登录用户端并访问用户端相关接口，提升用户体验
- **Target Users**: 平台用户（使用用户端的普通用户）

## Goals
- 修复用户端登录后跳转到商家登录页的问题
- 修复用户端接口403权限错误的问题
- 确保用户端登录后能正常访问用户端相关接口
- 保持商家端功能不受影响
...
- 问题现象：用户使用13412340002和123456登录用户端后，会从用户首页跳转到商家登录页，且用户端接口返回403错误
- 当前状态：已初步分析并修复了部分问题，需要进一步验证

## Functional Requirements
- **FR-1**: 修复用户端登录后路由跳转逻辑，确保登录后停留在用户首页
- **FR-2**: 修复前端API请求拦截器，确保正确传递用户token
- **FR-3**: 确保用户端接口能正常访问，返回200 OK
- **FR-4**: 保持商家端登录和接口访问功能正常

## Non-Functional Requirements
- **NFR-1**: 性能：修复后登录跳转时间不超过2秒
- **NFR-2**: 可靠性：修复后用户端登录成功率达到100%
...
## Assumptions
- 后端API本身功能正常，问题出在前端的token传递和路由跳转
- 用户账号13412340002是有效的用户账号，不是商家账号
- 后端权限控制逻辑正确，问题出在前端的认证信息传递

## Acceptance Criteria

### AC-1: 用户端登录后不跳转到商家登录页
- **Given**: 用户在用户端登录页面输入正确的用户名和密码
- **When**: 点击登录按钮并登录成功
- **Then**: 页面跳转到用户首页（/user/home），且不再跳转到商家登录页
...
- **Given**: 用户已成功登录用户端
- **When**: 发起API请求
- **Then**: 请求头中正确携带用户token，且不携带商家token
- **Verification**: `programmatic`

## Open Questions
- [x] 前端请求拦截器的token优先级是否正确？
- [x] 后端CORS配置是否完整？
- [x] Mock服务是否干扰真实API请求？
- [ ] 前端路由守卫逻辑是否正确？