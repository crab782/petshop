...
describe('Login.vue - 平台端登录页面', () => {
  ...
  describe('页面渲染测试', () => {
    it('应该正确渲染登录容器', () => {
      expect(wrapper.find('.admin-login-container').exists()).toBe(true)
    ...
    it('应该正确渲染登录标题和副标题', () => {
      expect(wrapper.find('.login-title').text()).toBe('平台端登录')
      expect(wrapper.find('.login-subtitle').text()).toBe('欢迎回来，管理员')
    })
    ...
    it('应该渲染平台端特色功能', () => {
      expect(wrapper.find('.admin-features').exists()).toBe(true)
  ...
  describe('登录功能测试', () => {
    ...
    it('登录成功后应该保存token和管理员信息', async () => {
      ...
      expect(JSON.parse(localStorage.getItem('adminInfo') || '{}')).toEqual({ id: 1, username: 'admin' })
    })
  ...
  describe('导航功能测试', () => {
    it('点击注册链接应该跳转到平台注册页', () => {
      ...
      expect(mockRouterPush).toHaveBeenCalledWith('/admin/register')
    })
  ...
  describe('安全性测试', () => {
    ...
    it('登录成功后token应该存储在localStorage', async () => {
      ...
      expect(localStorage.getItem('admin_token')).toBe('secure-admin-token-123')
    })
  })
  ...
  describe('平台管理特有功能测试', () => {
    it('应该显示平台端特色功能', () => {
      ...
      expect(featureTexts[0].text()).toBe('用户管理')
      expect(featureTexts[1].text()).toBe('商家管理')
      expect(featureTexts[2].text()).toBe('系统设置')
    })

    it('登录成功后应该跳转到平台首页', async () => {
      const mockRequestPost = vi.mocked(request.post)
      mockRequestPost.mockResolvedValueOnce({
        token: 'admin-token',
        user: { id: 1 }
