...
class HTTPClient:
    ...
    def get(
        self, endpoint: str, params: Dict = None, user_type: str = None, headers: Dict = None, **kwargs
    ) -> requests.Response:
        ...
        response = self.session.get(url, params=params, headers=request_headers, timeout=self.timeout, **kwargs)
    ...
    def post(
        ...
        **kwargs,
        ...
        response = self.session.post(
            url, data=data, json=json_data, headers=request_headers, timeout=self.timeout, **kwargs
        )
    ...
    def delete(self, endpoint: str, user_type: str = None, headers: Dict = None, **kwargs) -> requests.Response:
        url = f"{self.base_url}{endpoint}"
        request_headers = self._get_headers(user_type, headers)

        logger.info(f"DELETE {url}")

        response = self.session.delete(url, headers=request_headers, timeout=self.timeout, **kwargs)
...
def assert_json_key_exists(response: requests.Response, key: str) -> None:
    ...
    assert key in json_data, f"Key '{key}' not found in response: {json.dumps(json_data, ensure_ascii=False)}"
...
def assert_json_contains(response: requests.Response, expected_dict: Dict) -> None:
    ...
    for key, value in expected_dict.items():
        assert key in json_data, f"Key '{key}' not found in response"
        assert json_data[key] == value, f"Expected '{key}' to be {value}, but got {json_data[key]}"
