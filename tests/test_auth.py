...
class TestUserAuthentication:
    ...
    def test_user_login_wrong_password(self, http_client: HTTPClient, user_credentials: dict):
        ...
        """
        response = http_client.post(
            "/api/auth/login", json_data={"phone": user_credentials["phone"], "password": "wrongpassword123"}
        )

        assert response.status_code == 401, f"预期状态码401，实际状态码: {response.status_code}"
    ...
    def test_user_login_nonexistent(self, http_client: HTTPClient):
        ...
        """
        response = http_client.post(
            "/api/auth/login", json_data={"username": "nonexistent_user_12345", "password": "somepassword123"}
        )
...
class TestMerchantAuthentication:
    ...
    def test_merchant_login_success(self, http_client: HTTPClient, merchant_credentials: dict):
        ...
        response = http_client.post(
            ...
            json_data={"username": merchant_credentials["email"], "password": merchant_credentials["password"]},
        )
    ...
    def test_merchant_login_wrong_password(self, http_client: HTTPClient, merchant_credentials: dict):
        ...
        response = http_client.post(
            "/api/auth/merchant/login",
            json_data={"username": merchant_credentials["email"], "password": "wrongpassword123"},
        )
...
class TestTokenValidation:
    ...
    def test_token_validation(self, http_client: HTTPClient, user_token: str):
        ...
        response = http_client.get("/api/user/profile", user_type="user")

        assert response.status_code == 200, f"使用有效Token访问受保护接口失败，状态码: {response.status_code}"
...
class TestAuthenticationFlow:
    ...
    def test_complete_auth_flow(self, http_client: HTTPClient, user_credentials: dict):
        ...
        profile_response = http_client.get("/api/user/profile", user_type="user")
    ...
    def test_merchant_complete_auth_flow(self, http_client: HTTPClient, merchant_credentials: dict):
        ...
        profile_response = http_client.get("/api/merchant/profile", user_type="merchant")
...
class TestAuthenticationEdgeCases:
    ...
    def test_empty_username_login(self, http_client: HTTPClient):
        ...
        2. 验证返回400或错误信息
        """
        response = http_client.post("/api/auth/login", json_data={"username": "", "password": "somepassword123"})

        assert response.status_code in [400, 401], f"预期状态码400或401，实际状态码: {response.status_code}"
    ...
    def test_empty_password_login(self, http_client: HTTPClient, user_credentials: dict):
        ...
        2. 验证返回400或错误信息
        """
        response = http_client.post("/api/auth/login", json_data={"phone": user_credentials["phone"], "password": ""})
    ...
    def test_missing_required_fields(self, http_client: HTTPClient):
        ...
        2. 验证返回400或错误信息
        """
        response = http_client.post("/api/auth/login", json_data={"username": "testuser"})
    ...
    def test_sql_injection_protection(self, http_client: HTTPClient):
        ...
        for payload in malicious_payloads:
            response = http_client.post("/api/auth/login", json_data={"username": payload, "password": "anypassword"})
            ...
            assert "sql" not in response_text, f"响应中包含SQL关键字，可能存在SQL注入漏洞: {payload}"
            assert "error" not in response_text or "invalid" in response_text, f"响应中包含敏感错误信息: {payload}"
...
class TestTokenManagement:
    ...
    def test_token_refresh(self, http_client: HTTPClient, user_token: str):
        ...
        response = http_client.post("/api/auth/refresh", user_type="user")
    ...
    def test_token_logout(self, http_client: HTTPClient, user_token: str):
        ...
        logout_response = http_client.post("/api/auth/logout", user_type="user")

        if logout_response.status_code == 200:
            profile_response = http_client.get("/api/user/profile", user_type="user")

            assert profile_response.status_code == 401, "登出后Token仍然有效"
        else:
            pytest.skip("登出接口不可用")
