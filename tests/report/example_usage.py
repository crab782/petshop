...
def example_html_report():
    ...
    test_results = [
        TestResult(name="test_user_login_success", status="passed", duration=0.45),
        TestResult(name="test_user_login_invalid_password", status="passed", duration=0.32),
        TestResult(
        ...
        TestResult(name="test_create_order", status="passed", duration=0.89),
        TestResult(name="test_payment_integration", status="skipped", duration=0.01),
        TestResult(name="test_product_search", status="passed", duration=0.67),
        TestResult(
    ...
    report_file = html_gen.generate(
        test_results, output_path, title="宠物服务平台 - 测试报告", project_name="宠物服务平台"
    )
...
def example_parse_pytest_json():
    ...
    pytest_json = {
        "tests": [
            {"name": "test_api_user_login", "outcome": "passed", "duration": 0.56},
            {
            ...
            {"name": "test_api_create_order", "outcome": "passed", "duration": 0.78},
        ]
    }
...
def example_performance_report():
    ...
    report_file = perf_gen.generate(metrics, title="宠物服务平台 - 性能测试报告")
...
def example_coverage_report():
    ...
    report_file = cov_gen.generate(coverage_summary, title="宠物服务平台 - 代码覆盖率报告")
