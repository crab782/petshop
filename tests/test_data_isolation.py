...
from tests.fixtures.test_data import TestData, TestDataGenerator
...
class TestUserDataIsolation:
    ...
    def test_user_cannot_access_other_user_orders(self, http_client: HTTPClient):
        ...
        try:
            ...
            response = http_client.get(f"/api/user/orders/{order_a['id']}", user_type="user_b")

            assert response.status_code in [403, 404], f"用户B不应能访问用户A的订单，实际状态码: {response.status_code}"
    ...
    def test_user_cannot_access_other_user_appointments(self, http_client: HTTPClient):
        ...
        try:
            ...
            response = http_client.get(f"/api/user/appointments/{appointment_a['id']}", user_type="user_b")

            assert response.status_code in [403, 404], f"用户B不应能访问用户A的预约，实际状态码: {response.status_code}"
    ...
    def test_user_cannot_access_other_user_pets(self, http_client: HTTPClient):
        ...
        try:
            ...
            response = http_client.get(f"/api/user/pets/{pet_a['id']}", user_type="user_b")

            assert response.status_code in [403, 404], f"用户B不应能访问用户A的宠物，实际状态码: {response.status_code}"
            ...
            update_response = http_client.put(
                ...
                json_data={"name": "modified_by_other_user", "type": "狗", "breed": "金毛", "age": 3, "gender": "male"},
                user_type="user_b",
            )
    ...
    def test_user_cannot_access_other_user_addresses(self, http_client: HTTPClient):
        ...
        try:
            ...
            response = http_client.get(f"/api/user/addresses/{address_a['id']}", user_type="user_b")

            assert response.status_code in [403, 404], f"用户B不应能访问用户A的地址，实际状态码: {response.status_code}"
...
class TestMerchantDataIsolation:
    ...
    def test_merchant_cannot_access_other_merchant_orders(self, http_client: HTTPClient):
        ...
        try:
            ...
            response = http_client.get(f"/api/merchant/orders/{order_a['id']}", user_type="merchant_b")

            assert response.status_code in [403, 404], f"商家B不应能访问商家A的订单，实际状态码: {response.status_code}"
    ...
    def test_merchant_cannot_access_other_merchant_appointments(self, http_client: HTTPClient):
        ...
        try:
            ...
            response = http_client.get(f"/api/merchant/appointments/{appointment_a['id']}", user_type="merchant_b")

            assert response.status_code in [403, 404], f"商家B不应能访问商家A的预约，实际状态码: {response.status_code}"
    ...
    def test_merchant_cannot_access_other_merchant_services(self, http_client: HTTPClient):
        ...
        try:
            ...
            response = http_client.get(f"/api/merchant/services/{service_a['id']}", user_type="merchant_b")

            assert response.status_code in [403, 404], f"商家B不应能访问商家A的服务，实际状态码: {response.status_code}"
            ...
            delete_response = http_client.delete(f"/api/merchant/services/{service_a['id']}", user_type="merchant_b")
    ...
    def test_merchant_cannot_access_other_merchant_products(self, http_client: HTTPClient):
        ...
        try:
            ...
            response = http_client.get(f"/api/merchant/products/{product_a['id']}", user_type="merchant_b")

            assert response.status_code in [403, 404], f"商家B不应能访问商家A的商品，实际状态码: {response.status_code}"
...
class TestRoleIsolation:
    ...
    def test_user_cannot_access_admin_api(self, http_client: HTTPClient):
        ...
        try:
            ...
            admin_endpoints = ["/api/admin/users", "/api/admin/merchants", "/api/admin/services", "/api/admin/products"]
    ...
    def test_user_cannot_modify_other_user_data(self, http_client: HTTPClient):
        ...
        try:
            ...
            update_pet_response = http_client.put(
                f"/api/user/pets/{pet_a['id']}",
                json_data={"name": "hacked_pet", "type": "猫", "breed": "英短", "age": 2, "gender": "female"},
                user_type="user_b",
            )

            assert update_pet_response.status_code in [
                403,
                404,
            ...
            assert update_address_response.status_code in [
                403,
                404,
            ], f"用户B不应能修改用户A的地址，实际状态码: {update_address_response.status_code}"

            delete_pet_response = http_client.delete(f"/api/user/pets/{pet_a['id']}", user_type="user_b")

            assert delete_pet_response.status_code in [
                403,
                404,
    ...
    def test_merchant_cannot_modify_other_merchant_data(self, http_client: HTTPClient):
        ...
        try:
            ...
            update_service_response = http_client.put(
                f"/api/merchant/services/{service_a['id']}",
                json_data={"name": "hacked_service", "description": "被其他商家修改", "price": 0.01, "duration": 1},
                user_type="merchant_b",
            )

            assert update_service_response.status_code in [
                403,
                404,
            ...
            update_product_response = http_client.put(
                f"/api/merchant/products/{product_a['id']}",
                json_data={"name": "hacked_product", "description": "被其他商家修改", "price": 0.01, "stock": 99999},
                user_type="merchant_b",
            )

            assert update_product_response.status_code in [
                403,
                404,
            ], f"商家B不应能修改商家A的商品，实际状态码: {update_product_response.status_code}"

            delete_service_response = http_client.delete(
                f"/api/merchant/services/{service_a['id']}", user_type="merchant_b"
            )

            assert delete_service_response.status_code in [
                403,
                404,
            ], f"商家B不应能删除商家A的服务，实际状态码: {delete_service_response.status_code}"

            delete_product_response = http_client.delete(
                f"/api/merchant/products/{product_a['id']}", user_type="merchant_b"
            )

            assert delete_product_response.status_code in [
                403,
                404,
...
class TestCrossRoleDataAccess:
    ...
    def test_merchant_cannot_access_user_data_as_user(self, http_client: HTTPClient):
        ...
        try:
            ...
            response = http_client.get(f"/api/user/pets/{pet['id']}", user_type="merchant")
    ...
    def test_user_cannot_see_other_user_sensitive_info(self, http_client: HTTPClient):
        ...
        try:
            ...
            if "data" in profile_data:
                ...
                assert "password" not in user_data or user_data["password"] is None, "响应中不应包含密码字段"

                if "phone" in user_data and user_data["phone"]:
                    phone = user_data["phone"]
                    assert phone != user_a.get("phone"), "不应能看到其他用户的完整手机号"
