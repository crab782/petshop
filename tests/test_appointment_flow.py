...
class TestAppointmentFlow:
    ...
    def test_create_appointment(self, http_client: HTTPClient, test_data: TestData):
        ...
        assert response.status_code in [200, 201], \
            f"创建预约失败，状态码: {response.status_code}"
        
        response_data = response.json()
        assert "data" in response_data, "响应中没有data字段"
        
        appointment_id = response_data.get("data", {}).get("id") or \
                        response_data.get("data", {}).get("appointmentId")
        
        assert appointment_id is not None, "返回的预约ID为空"
        
        appointment_status = response_data.get("data", {}).get("status")
        assert appointment_status == "pending", \
            f"预约状态应为pending，实际为: {appointment_status}"
    ...
    def test_merchant_confirm_appointment(self, http_client: HTTPClient, test_data: TestData):
        ...
        assert response.status_code == 200, \
            f"商家确认预约失败，状态码: {response.status_code}"
        
        response_data = response.json()
        updated_status = response_data.get("data", {}).get("status")
        
        assert updated_status == "confirmed", \
            f"预约状态应为confirmed，实际为: {updated_status}"
    ...
    def test_merchant_complete_appointment(self, http_client: HTTPClient, test_data: TestData):
        ...
        assert response.status_code == 200, \
            f"商家完成服务失败，状态码: {response.status_code}"
        
        response_data = response.json()
        updated_status = response_data.get("data", {}).get("status")
        
        assert updated_status == "completed", \
            f"预约状态应为completed，实际为: {updated_status}"
    ...
    def test_user_cancel_pending_appointment(self, http_client: HTTPClient, test_data: TestData):
        ...
        response = http_client.put(
            f"/api/user/appointments/{appointment_id}/cancel",
            user_type="user"
        )
        
        assert response.status_code == 200, \
            f"用户取消预约失败，状态码: {response.status_code}"
        
        response_data = response.json()
        updated_status = response_data.get("data", {}).get("status")
        
        assert updated_status == "cancelled", \
            f"预约状态应为cancelled，实际为: {updated_status}"
    ...
    def test_merchant_cancel_confirmed_appointment(self, http_client: HTTPClient, test_data: TestData):
        ...
        assert response.status_code == 200, \
            f"商家取消预约失败，状态码: {response.status_code}"
    ...
    def test_complete_appointment_flow(self, http_client: HTTPClient, test_data: TestData):
        ...
        get_response = http_client.get(
            f"/api/user/appointments/{appointment_id}",
            user_type="user"
        )
        ...
        assert confirmed_status == "confirmed", \
            f"确认后状态应为confirmed，实际为: {confirmed_status}"
        ...
        assert completed_status == "completed", \
            f"完成后状态应为completed，实际为: {completed_status}"
