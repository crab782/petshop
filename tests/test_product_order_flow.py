...
class TestProductOrderFlow:
    ...
    def test_create_product_order(self, http_client: HTTPClient, test_data: TestData):
        ...
        response = http_client.post("/api/user/orders", json_data=order_data, user_type="user")

        assert response.status_code in [200, 201], f"创建订单失败，状态码: {response.status_code}"

        response_data = response.json()
        assert "data" in response_data, "响应中没有data字段"

        order_id = response_data.get("data", {}).get("id") or response_data.get("data", {}).get("orderId")
        ...
        assert order_status == "pending", f"订单状态应为pending，实际为: {order_status}"
    ...
    def test_user_pay_order(self, http_client: HTTPClient, test_data: TestData):
        ...
        pay_data = {"payMethod": "wechat", "transactionId": f"TXN{datetime.now().strftime('%Y%m%d%H%M%S')}"}

        response = http_client.put(f"/api/user/orders/{order_id}/pay", json_data=pay_data, user_type="user")

        assert response.status_code == 200, f"用户支付订单失败，状态码: {response.status_code}"

        response_data = response.json()
        updated_status = response_data.get("data", {}).get("status")

        assert updated_status == "paid", f"订单状态应为paid，实际为: {updated_status}"
    ...
    def test_merchant_ship_order(self, http_client: HTTPClient, test_data: TestData):
        ...
        pay_response = http_client.put(
            ...
            json_data={"payMethod": "wechat", "transactionId": f"TXN{datetime.now().strftime('%Y%m%d%H%M%S')}"},
            user_type="user",
        )

        assert pay_response.status_code == 200, "支付订单失败"

        ship_data = {"logisticsCompany": "顺丰速运", "logisticsNumber": f"SF{datetime.now().strftime('%Y%m%d%H%M%S')}"}

        response = http_client.put(f"/api/merchant/orders/{order_id}/ship", json_data=ship_data, user_type="merchant")

        assert response.status_code == 200, f"商家发货失败，状态码: {response.status_code}"

        response_data = response.json()
        updated_status = response_data.get("data", {}).get("status")

        assert updated_status == "shipped", f"订单状态应为shipped，实际为: {updated_status}"
    ...
    def test_user_confirm_receipt(self, http_client: HTTPClient, test_data: TestData):
        ...
        response = http_client.put(f"/api/user/orders/{order_id}/confirm", user_type="user")

        assert response.status_code == 200, f"用户确认收货失败，状态码: {response.status_code}"

        response_data = response.json()
        updated_status = response_data.get("data", {}).get("status")

        assert updated_status == "completed", f"订单状态应为completed，实际为: {updated_status}"
    ...
    def test_user_cancel_pending_order(self, http_client: HTTPClient, test_data: TestData):
        ...
        response = http_client.put(f"/api/user/orders/{order_id}/cancel", user_type="user")

        assert response.status_code == 200, f"用户取消订单失败，状态码: {response.status_code}"

        response_data = response.json()
        updated_status = response_data.get("data", {}).get("status")

        assert updated_status == "cancelled", f"订单状态应为cancelled，实际为: {updated_status}"
    ...
    def test_merchant_cancel_paid_order(self, http_client: HTTPClient, test_data: TestData):
        ...
        response = http_client.put(
            f"/api/merchant/orders/{order_id}/cancel", json_data={"reason": "缺货退款"}, user_type="merchant"
        )

        assert response.status_code == 200, f"商家取消订单失败，状态码: {response.status_code}"
    ...
    def test_invalid_status_transition_completed(self, http_client: HTTPClient, test_data: TestData):
        ...
        confirm_response = http_client.put(f"/api/user/orders/{order_id}/confirm", user_type="user")
        assert confirm_response.status_code == 200, "确认收货失败"

        response = http_client.put(
            f"/api/merchant/orders/{order_id}/cancel", json_data={"reason": "测试取消"}, user_type="merchant"
        )
    ...
    def test_invalid_status_transition_cancelled(self, http_client: HTTPClient, test_data: TestData):
        ...
        cancel_response = http_client.put(f"/api/user/orders/{order_id}/cancel", user_type="user")
    ...
    def test_complete_product_order_flow(self, http_client: HTTPClient, test_data: TestData):
        ...
        get_response = http_client.get(f"/api/user/orders/{order_id}", user_type="user")
        ...
        assert paid_status == "paid", f"支付后状态应为paid，实际为: {paid_status}"
        ...
        assert shipped_status == "shipped", f"发货后状态应为shipped，实际为: {shipped_status}"

        confirm_response = http_client.put(f"/api/user/orders/{order_id}/confirm", user_type="user")

        assert confirm_response.status_code == 200, "确认收货失败"
        completed_status = confirm_response.json().get("data", {}).get("status")
        assert completed_status == "completed", f"确认收货后状态应为completed，实际为: {completed_status}"
...
class TestProductOrderEdgeCases:
    ...
    def test_create_order_nonexistent_address(self, http_client: HTTPClient, test_data: TestData):
        ...
        order_data = {
            "items": [{"productId": test_data.product_ids[0], "quantity": 1}],
            "addressId": 99999,
            "notes": "测试订单",
        }
    ...
    def test_create_order_zero_quantity(self, http_client: HTTPClient, test_data: TestData):
        ...
        order_data = {
            "items": [{"productId": test_data.product_ids[0], "quantity": 0}],
            "addressId": test_data.address_ids[0],
            "notes": "测试订单",
        }
    ...
    def test_create_order_missing_fields(self, http_client: HTTPClient):
        ...
        2. 验证返回错误
        """
        order_data = {"addressId": 1, "notes": "测试订单"}
    ...
    def test_pay_order_not_pending(self, http_client: HTTPClient, test_data: TestData):
        ...
        response = http_client.put(
            f"/api/user/orders/{order_id}/pay",
            json_data={"payMethod": "alipay", "transactionId": f"TXN{datetime.now().strftime('%Y%m%d%H%M%S')}"},
            user_type="user",
        )
    ...
    def test_create_order_multiple_products(self, http_client: HTTPClient, test_data: TestData):
        ...
        order_data = {
            "items": [
                {"productId": test_data.product_ids[0], "quantity": 2},
                {"productId": test_data.product_ids[1], "quantity": 1},
            ],
            "addressId": test_data.address_ids[0],
            "notes": "测试多商品订单",
        }
        ...
        assert response.status_code in [200, 201], f"创建多商品订单失败，状态码: {response.status_code}"
