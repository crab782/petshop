...
class ErrorLogger:
    ...
    def __init__(self, log_dir: str = None):
        ...
        self.error_logger = self._setup_logger(
            "error_logger",
            self.log_dir / AutoFixConfig.LOG_CONFIG["error_log_file"]
        )
        
        self.retry_logger = self._setup_logger(
            "retry_logger",
            self.log_dir / AutoFixConfig.LOG_CONFIG["retry_log_file"]
        )
...
class FailedTestDetector:
    ...
    def _extract_traceback(self, output: str, test_name: str) -> str:
        traceback_pattern = re.compile(
            rf'{re.escape(test_name)}.*?(?={re.escape(test_name)}|FAILED|PASSED|=+$)',
            re.DOTALL
        )
    ...
    def extract_request_response(self, failure: TestFailure, test_context: Dict = None) -> Tuple[Optional[Dict], Optional[Dict]]:
        if test_context:
...
class AutoFixRunner:
    ...
    def _generate_final_report(self):
        ...
        self.logger.save_error_report(
            self.failures,
            report_dir / AutoFixConfig.REPORT_CONFIG["json_report"]
        )
