...
import { ElButton, ElTable, ElTableColumn, ElDialog, ElForm, ElFormItem, ElInput, ElMessage, ElMessageBox, ElSwitch, ElPagination, ElInputNumber, ElImage } from 'element-plus'
import { Plus, Edit, Delete, Search, Refresh } from '@element-plus/icons-vue'
import { getCategories, addCategory, updateCategory, deleteCategory, updateCategoryStatus, batchUpdateCategoryStatus, batchDeleteCategories, type Category } from '@/api/merchant'
...
const handleBatchEnable = async () => {
  ...
  try {
    ...
    await batchUpdateCategoryStatus(ids, 'enabled')
...
const handleBatchDisable = async () => {
  ...
  try {
    const ids = selectedRows.value.map(row => row.id)
    await batchUpdateCategoryStatus(ids, 'disabled')
...
const handleSortChange = async (row: Category, direction: number) => {
  ...
  try {
    await updateCategory(row.id, { sort: newSort })
...
<template>
  <div class="merchant-categories">
    ...
    <el-table :data="paginatedCategories" v-loading="loading" style="width: 100%" @selection-change="handleSelectionChange">
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px" destroy-on-close>
      <el-form ref="formRef" :model="formData" :rules="rules" label-width="80px">
        ...
        <el-form-item label="分类图标">
          <el-input v-model="formData.icon" placeholder="请输入图标URL或上传图片" />
        </el-form-item>
